"""
Ultimate ASCII / ANSI Studio Bot — Complete Edition (Fixed)
----------------------------------------------------------
- Auto pip-installs dependencies on first run
- Uses modern importlib (no more pkgutil.find_loader error)
- Prints clear /start instructions to the terminal
- 33 named colors + custom hex
...
ALLOWED_CHAT_ID = os.getenv("ALLOWED_CHAT_ID", "7669164275").strip()   # empty = allow all
...
def main() -> None:
    if not BOT_TOKEN or BOT_TOKEN.startswith("123456789:"):
        ...
        print("        Set the environment variable BOT_TOKEN.")
